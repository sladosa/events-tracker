...
- 🐛 BUG #1 FIXED: Filters not disabling after 2nd+ edit operation
  - ROOT CAUSE: Filters rendered BEFORE tabs detect changes
  - Tabs set has_changes=True but filters already rendered with old state
  - SOLUTION: Track previous_has_changes, rerun when state changes for sync
  - Added filter_sync_needed flag and UI synchronization logic
- 🐛 BUG #2 CLARIFIED: Add forms don't lock filters - this is EXPECTED behavior
  - Forms use st.form() which doesn't create "unsaved changes" until submit
  - User can navigate away from form without losing data elsewhere
  - Added info message explaining this behavior
- 🐛 BUG #3 FIXED: Discard button in Add New Category infinite loop
  - ROOT CAUSE: Button had FIXED key, Streamlit remembered click state
  - SOLUTION: Dynamic key with editor_reset_counter for all form Discard buttons
  - Pattern: key=f"discard_category_form_{st.session_state.editor_reset_counter}"
- 🐛 BUG #4 FIXED: Insert Category Between not working
  - Added better validation and error handling
  - Fixed area_id extraction from filtered DataFrame
  - Added debug logging for troubleshooting
- 🐛 BUG #5 FIXED: Discard in Insert Category Between infinite loop
  - Same fix as BUG #3 - dynamic button key
- 🔧 IMPROVED: DEBUG INFO sidebar
  - Added previous_has_changes tracking
  - Added filter_sync_needed flag display
  - Added any_tab_has_changes tracking
  - Better visibility into state machine operation
- 🎯 IMPACT:
  - Filters now properly disable/enable on ALL edit operations ✅
  - No more infinite loops on ANY Discard button ✅
  - Insert Category Between works correctly ✅
...
def remove_category_between(
    ...
    FIXED: Removed grandchildren check to allow multi-level subtrees.
    Promotes ALL direct children (handles multiple like Gym + Outdoor).
    """
    try:
        # 1. Get category info
        ...
        # 2. Get direct children (Gym, Outdoor, etc.)
        children = (client.table('categories')
                    .select('id, name')
        ...
        # Name conflict check (zadržano za sigurnost)
        if parentid:
            existingsiblings = (client.table('categories')
                                .select('name')
            ...
            existingsiblings = (client.table('categories')
                                ...
                                .neq('id', categoryid)
                                .execute())
        existingnames = set(s['name'] for s in existingsiblings.data or [])
        
        for child in childrendata:
            if child['name'] in existingnames:
                return False, f"Cannot promote {child['name']} - name conflict under target parent"
        
        # Promote ALL children (Gym keeps Cardio/Strength)
        promotedchildren = []
        for child in childrendata:
            updateresult = (client.table('categories')
                            .update({
                                'parentcategoryid': parentid,
                                'level': catlevel  # Level removed category
                            })
                            .eq('id', child['id'])
                            .eq('userid', userid)
                            .execute())
        ...
        # Delete direct attributes/events (ne dira djecu)
        ...
        msg = f"Removed '{catname}' and promoted {childrencount} children: {', '.join(promotedchildren)}"
        return True, msg
...
def render_interactive_structure_viewer(client, user_id: str):
    ...
    **Quick Overview:**
    ...
    if st.session_state.viewer_mode == 'read_only':
        ...
        # v1.12.0: Track previous state for filter synchronization
        # This is CRITICAL for fixing the "filters don't disable on 2nd edit" bug
        previous_has_changes = state_mgr.state.has_changes
        
        # v1.12.0: Track if ANY tab has changes (accumulated through all tabs)
        # This replaces the problematic reset on line 2429
        any_tab_has_changes = False
        ...
        with tab1:
            ...
            if area_full_df.empty:
                ...
                # v1.12.0: Accumulate changes across tabs (don't reset, just add)
                # This fixes the filter sync issue
                if has_area_edit_changes:
                    any_tab_has_changes = True
                ...
                with st.expander("➕ Add New Area", expanded=False):
                    ...
                    # v1.12.0: Dynamic key prevents infinite loop (Streamlit button state issue)
                    if st.button("🗑️ Discard", key=f"discard_area_form_{st.session_state.editor_reset_counter}", help="Close form and clear inputs"):
                        st.session_state.area_form_counter += 1
        ...
        with tab2:
            ...
            with st.expander("➕ Add New Category", expanded=False):
                ...
                # v1.12.0: Discard button with dynamic key (prevents infinite loop)
                if st.button("🗑️ Discard", key=f"discard_category_form_{st.session_state.editor_reset_counter}", help="Close form and clear inputs"):
                    st.session_state.category_form_counter += 1
                    # Clear any potential change detection state
                    st.cache_data.clear()
                    st.session_state.original_df = None
            ...
            with st.expander("📌 Insert Category Between", expanded=False):
                ...
                **Example:** If you have `Automobili > A_test > A_test2`  
                ...
                if categories_in_area_df.empty:
                    ...
                    # v1.12.0: Discard button with dynamic key (prevents infinite loop)
                    if st.button("🗑️ Discard", key=f"discard_insert_form_{st.session_state.editor_reset_counter}", help="Close form and clear inputs"):
                        st.session_state.insert_between_counter += 1
                        # Clear any potential change detection state
                        st.cache_data.clear()
                        st.session_state.original_df = None
        ...
        with tab3:
            ...
            with st.expander("➕ Add New Attribute", expanded=False):
                ...
                # v1.12.0: Discard button with dynamic key (prevents infinite loop)
                if st.button("🗑️ Discard", key=f"discard_attribute_form_{st.session_state.editor_reset_counter}", help="Close form and clear inputs"):
                    st.session_state.attribute_form_counter += 1
                    # Clear any potential change detection state
                    st.cache_data.clear()
                    st.session_state.original_df = None
...
| 🟦 BLUE | Category, Attribute_Name, Data_Type, Unit, etc. | Edit freely |
        ...
        # ============================================
        # v1.12.0: END OF EDIT MODE - STATE SYNC
        # ============================================
        # This is the CRITICAL section that ensures filters reflect actual state
        
        # 1. Update has_changes based on accumulated tab results
        state_mgr.state.has_changes = any_tab_has_changes
        
        # 2. Clear discard_pending flag (persists for ONE render cycle)
        if state_mgr.state.discard_pending:
            state_mgr.state.discard_pending = False
        
        # 3. CHECK IF FILTER SYNC IS NEEDED
        # If has_changes state changed, we need to rerun so filters reflect new state
        # This fixes BUG #1: filters not disabling after 2nd+ edit
        current_has_changes = state_mgr.state.has_changes
        if current_has_changes != previous_has_changes:
            # State changed - filters need to update
            # Rerun ONCE to sync filters with new state
            st.rerun()
