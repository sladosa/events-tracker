...
Last Modified: 2025-11-30 15:00 UTC
Python: 3.11
Version: 2.0.1 - FIXED Export Filtered + Removed Refresh from Read-Only
...
FIXES in v2.0.1:
- ✅ REMOVED Refresh button from Read-Only mode (caused issues)
- ✅ ADDED Export Filtered button in Read-Only mode
- ✅ Export respects Area and Search filters
- ✅ Refresh button ONLY in Edit mode now
...
COLUMN_CONFIG = [
    ...
    ('Category_Path', False, 'text'),  # Auto-calculated
    ...
    try:
        ...
        for area in areas:
            ...
            for root_cat in root_categories:
                _add_category_tree(
                    root_cat, area_name, area_name, rows,
                    categories_by_parent, attributes_by_category, categories_by_id
                )
...
def _add_category_tree(category, area_name, parent_path, rows, categories_by_parent, attributes_by_category, categories_by_id):
    ...
    for child_cat in child_categories:
        _add_category_tree(
            child_cat, area_name, cat_path, rows,
            categories_by_parent, attributes_by_category, categories_by_id
        )
...
def save_changes_to_database(client, user_id: str, original_df: pd.DataFrame, edited_df: pd.DataFrame) -> Tuple[bool, str, Dict[str, int]]:
    ...
    try:
        ...
        if stats['errors'] > 0:
            return False, f"Completed with {stats['errors']} errors", stats
...
def add_new_category(client, user_id: str, area_id: str, name: str, description: str = "", parent_category_id: Optional[str] = None) -> Tuple[bool, str]:
...
def add_new_attribute(client, user_id: str, category_id: str, name: str, data_type: str, unit: str = "", is_required: bool = False, default_value: str = "", validation_min: str = "", validation_max: str = "", description: str = "") -> Tuple[bool, str]:
...
def render_interactive_structure_viewer(client, user_id: str):
    ...
    **Excel-like editing interface** - Direct database connectivity
    ...
    - 📥 **Export Filtered**: Download only filtered structure to Excel
    """)
    ...
    with col1:
        ...
        new_mode = st.radio("Mode", mode_options, index=current_mode_idx, horizontal=True)
    ...
    with col3:
        # FIXED: Export Filtered button for Read-Only, Refresh for Edit mode
        if st.session_state.viewer_mode == 'read_only':
            if st.button("📥 Export Filtered", use_container_width=True):
                with st.spinner("Generating Excel..."):
                    # Apply filters
                    filtered_export = apply_filters(df, selected_area, st.session_state.isv_search_text)
                    
                    # Remove metadata columns
                    export_cols = [col for col in filtered_export.columns if not col.startswith('_')]
                    filtered_export = filtered_export[export_cols]
                    
                    # Generate Excel
                    excel_bytes = export_structure_to_excel(filtered_export, format_type='hierarchical')
                    
                    # Generate filename
                    filter_suffix = f"_{selected_area.replace(' ', '_')}" if selected_area != "All Areas" else ""
                    filename = f"Structure_Filtered{filter_suffix}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
                    ...
                    st.download_button(
                        ...
                        data=excel_bytes,
                        file_name=filename,
                        mime=get_excel_mime_type(),
                        use_container_width=True,
                        key="download_filtered_excel"
                    )
                    
                    st.success(f"✅ Excel generated with {len(filtered_export)} rows!")
        else:
    ...
    st.session_state.isv_search_text = search_term
    ...
    if st.session_state.viewer_mode == 'read_only':
        ...
        st.info("⚠️ Full edit mode with tabs is not included in this version. Use Add/Delete functions below.")
